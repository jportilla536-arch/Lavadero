...
promotionsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const created = await run<unknown[]>(sb().from('promotions').insert(toRow(body)).select('*'));
...
promotionsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const updated = await run<unknown[]>(
      sb().from('promotions').update(patch).eq('id', req.params.id).select('*'),
    );
  ...
  asyncHandler(async (req, res) => {
    ...
    if ((count ?? 0) > 0) {
      const archived = await run<unknown[]>(
        sb().from('promotions').update({ active: false }).eq('id', req.params.id).select('*'),
      );
    ...
    const deleted = await run<unknown[]>(
      sb().from('promotions').delete().eq('id', req.params.id).select('id'),
    );
